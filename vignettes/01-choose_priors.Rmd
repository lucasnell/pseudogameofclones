...
$\sigma_\varepsilon$ was sampled as $\sim \text{N}(\theta_1, \theta_2) \texttt{T[0,]}$.
...
mean ($\rho \sim \text{N} ( \theta_3, \theta_4 )$) and
SD ($\sigma_\rho \sim \text{N} ( \theta_5, \theta_6 ) \texttt{T[0,]}$).
...
on three parameters:
overall mean ($\phi \sim \text{N}(\theta_7, ~ \theta_8)$),
among-line SD
...
```{r save_priors, eval = FALSE}
devtools::use_data(theta, overwrite = TRUE)
```
