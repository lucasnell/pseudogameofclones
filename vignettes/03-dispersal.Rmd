...
growth %>%
    ggplot(aes(line, log1p(dD))) +
    geom_point(shape = 1, position = position_jitter(height = 0, width = 0.25), alpha = 0.5) +
    scale_color_brewer(palette = "Set3", guide = FALSE) +
    NULL
...
growth %>%
    group_by(line, rep) %>% 
    summarize(disp = sum(disp) / sum(N)) %>% 
    ungroup() %>% 
    ggplot(aes(line, disp, color = line)) +
    geom_point(shape = 16, size = 3, alpha = 0.5,
               position = position_jitter(height = 0, width = 0.2)) +
    stat_summary(geom = "point", fun.y = mean, color = "black", size = 6, shape = 1) +
