...
# Number dispersed as function of line, with overdispersion:
disp_pois <- glmer(disp ~ 0 + line + (1 | od),
                   data = filter(growth, disp > 0),
                   family = poisson, offset = X)
disp_binom %>% summary()
disp_pois %>% summary()
```
...
Overdispersion estimate for the Poisson regression:

```{r make_disp_od}
disp_od <- VarCorr(disp_pois) %>%
    as.data.frame() %>% 
    .[["sdcor"]]
disp_od
```
...
if (!identical(disp_estimates$binom$line, disp_estimates$pois$line)) {
...
disp_estimates %>% 
    ...
    knitr::kable(
        ...
        caption = paste("Model estimates for the binomial GLM (top) and",
                        "Poisson GLMM (bottom), for each line.")
        )
```
...
predict_disp <- function(N, aphid_line) {
    ...
    # Poisson estimate (# dispersed | dispersal occurs):
    b0 <- disp_estimates$pois$inter[inds]
    X <- log(N)
    n_disp <- exp(b0 + X)
...
# Generating overdispersion:
over_pois <- function(n, lambda, sd) {
    return(rpois(n, lambda) * rlnorm(n, meanlog = 0, sdlog = sd))
}
```
