...
A <- apply(rstan::extract(stan_fit, "A", permuted = FALSE), 3, mean) %>%
    as.numeric()
D_df <- clonewars::disp_estimates
# Correcting for weirdness from WI-L4Ø due to only having one complete rep:
D_df[D_df$line == "WI-L4Ø",c("inter","X")] <- 
    colMeans(D_df[D_df$line != "WI-L4Ø", c("inter","X")])
...
```{r do_sims}
set.seed(654651984L)
sim_df <- cwsims::sim_cages(n_cages, N_0, max_t, R, A, D_df, process_error,
...
sim_by_cage %>% 
    ...
    mutate(disp = map_dbl(line,
                          function(i) {
                              inds <- disp_estimates$line == i
                              gtools::inv.logit(disp_estimates$inter[inds] + 
                                                    disp_estimates$X[inds] * log(500))
                          })) %>% 
    ggplot(aes(disp, n, color = line)) +
    geom_point(size = 3) +
    scale_color_brewer(palette = "Dark2", guide = FALSE) +
    ylab("Number of times survived") +
    xlab("Dispersal probability at N = 500") +
    NULL
