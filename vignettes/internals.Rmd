...
These data inform the default parameters (see `vignette("parameters", "gameofclones")`).
...
The code `c(mapply(..., FUN = rep), recursive = TRUE)` converts these values 
from per-instar to per-day, based on the 20º and 27ºC development rates.
...
* `rel_attack`: relative attack rate for low (20º C; `lowT`) and 
  high (27º C; `highT`) temperatures
...
* `disp_start`: when aphids disperse by flying, for low (20º C; `lowT`) and 
  high (27º C; `highT`) temperatures
