...
zeta <- rstan::extract(stan_fit, sprintf("zetas[%i]", 1:n_ts)) %>% 
    sapply(median) %>% 
    mean()
...
ricker <- function(t_hats, R, A, X0 = log(6)) {
    ...
    for (t in 2:length(t_hats)) {
        X[t] <- X[t-1] + R * (1 - A * exp(zeta * t_hats[t] + X[t-1]))
    }
    return(X)
}


t_mu <- mean(growth$date)
t_tau <- sd(growth$date)
...
pred_growth <- data_frame(time = rep(1:max_t, 8),
           ...
           R = rep(R_, each = max_t),
           A = rep(A_, each = max_t)) %>% 
    mutate(t_hat = (time - t_mu) / t_tau) %>% 
    group_by(line) %>% 
    mutate(X = ricker(t_hat, R[1], A[1])) %>% 
    ungroup()

mult <- exp(max(pred_growth$X)) / exp(zeta * max(pred_growth$t_hat))

pred_p <- pred_growth %>% 
    ggplot(aes(time, exp(X), color = line)) +
    geom_ribbon(data = pred_growth %>% distinct(time, t_hat),
              aes(ymin = 0, ymax = exp(zeta * t_hat) * mult, y = NULL),
...
pred_p
# ggsave("~/Desktop/pred_plot.pdf", pred_p, width = 6, height = 6)
```
