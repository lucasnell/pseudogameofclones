...
tau <- signif(log(0.005), 4)
sigma_tau <- 1
```


<!-- $c$ is bound $\ge 1$ and sampled as $\sim 1 + \text{Lognormal}(\tau, \sigma_\tau)$ -->
<!-- Based on simulations, an $c$ of 1.1 would be very high---so high as to be not -->
...
load_prior_data() %>% 
    ...
    #                       c_p = 1 + exp(tau)) %>% 
    #             as.numeric()) %>% 
    add_row(line = "sigma1", rep = rep(1:5, each=16), date = rep(0:15,5),
            X = sim_lines(R = 0.2, alpha = 1/1000, n_reps = 5, nobs_ts = rep(16, 5), 
                          c_p = 3) %>% 
                as.numeric()) %>% 
    mutate(rep = factor(rep), X = ifelse(X < 0, 0, X)) %>% 
    ggplot(aes(date, X)) +
