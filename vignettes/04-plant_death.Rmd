...
```{r after_max_data}
after_max <- load_data(filter_pars = NULL) %>% 
...
after_max %>%
    ...
    geom_line(aes(color = line, linetype = rep), alpha = 0.6) +
    stat_summary(fun.y = mean, geom = "point", size = 4, shape = 1) +
    stat_smooth(method = "lm", se = FALSE) +
    scale_color_brewer(palette = "Dark2", guide = FALSE) +
    scale_linetype_manual(guide = FALSE, values = 1:9) +
    NULL
...
after_max_mod <- after_max %>%
    ...
    {glm(cbind(N, N_dead) ~ 0 + date * line, family = "binomial", data = .)}
after_max_mod %>% 
    AIC()
...
after_max_coefs <- after_max_mod %>%
    coef() %>% 
    {
        ...
        rownames(..) <- gsub("line", "", names(.)[!grepl("^date", names(.))])
        ..
    } %>% 
    as.data.frame() %>% 
    set_names(c("b0", "b1")) %>%
