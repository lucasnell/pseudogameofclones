...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(88, 88, "end", "model_all_lines_plants");
    return reader;
}
...
public:
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        try {
            writer__.scalar_unconstrain(sigma_epsilon);
        } catch (const std::exception& e) { 
        ...
        try {
            writer__.scalar_unconstrain(sigma_rho);
        } catch (const std::exception& e) { 
        ...
        try {
            writer__.scalar_unconstrain(sigma_phi_a);
        } catch (const std::exception& e) { 
        ...
        try {
            writer__.scalar_unconstrain(sigma_phi_w);
        } catch (const std::exception& e) { 
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        try {
            ...
            if (jacobian__)
                sigma_epsilon = in__.scalar_constrain(lp__);
            else
                sigma_epsilon = in__.scalar_constrain();
            ...
            if (jacobian__)
                sigma_rho = in__.scalar_constrain(lp__);
            else
                sigma_rho = in__.scalar_constrain();
            ...
            if (jacobian__)
                sigma_phi_a = in__.scalar_constrain(lp__);
            else
                sigma_phi_a = in__.scalar_constrain();

            T__ sigma_phi_w;
            (void) sigma_phi_w;  // dummy to suppress unused var warning
            if (jacobian__)
                sigma_phi_w = in__.scalar_constrain(lp__);
            else
                sigma_phi_w = in__.scalar_constrain();
            ...
            for (int j = 1; j <= N_ts; ++j) {
                ...
                stan::model::assign(X_pred, 
                            ...
                            ricker(stan::model::rvalue(X, stan::model::cons_list(stan::model::index_omni(), stan::model::cons_list(stan::model::index_uni(j), stan::model::nil_index_list())), "X"),n_,r_,a_, pstream__), 
    ...
    void write_array(RNG& base_rng__,
        ...
        double sigma_epsilon = in__.scalar_constrain();
        double rho = in__.scalar_constrain();
        double sigma_rho = in__.scalar_constrain();
        double phi = in__.scalar_constrain();
        double sigma_phi_a = in__.scalar_constrain();
        double sigma_phi_w = in__.scalar_constrain();
