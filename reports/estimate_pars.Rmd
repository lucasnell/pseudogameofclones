...
```{r examine_model}
dim(rstan::extract(stan_fit, "R0", permuted = FALSE))
...
print(stan_fit, digits = 6, pars = c("R0", "alpha", "process"), probs = c())
print(stan_fit2, digits = 6, pars = c("R0", "alpha", "beta", "process"), probs = c())


pred_df <- make_pred_df(stan_fit, growth)
pred_df2 <- make_pred_df(stan_fit2, growth)
