...
if (rerun_sims) {
    ...
    df.save <- df
    df <- rbind(df, df.save)
    # write.csv(df, "_results/df_Fig4ab.csv", row.names = FALSE)
...
########################*
# df <- read.csv("_results/df_Fig4ab.csv")


# df2 <- read.csv("~/Stanford_Drive/UW/gameofclones/_drafts/2022-10-31_ARI/df_Fig4ab_26Oct22.csv")
...
points(prop.delta ~ disp, data = df[df$delta == 1 & df$lty.eig == 2 & !is.na(df$lty.eig),], lty = lty.eig)
points(peak.prop ~ disp, data = df[df$delta == 1 & !is.na(df$lty.eig),], lty = lty.eig)
points(trough.prop ~ disp, data = df[df$delta == 1 & !is.na(df$lty.eig),], lty = lty.eig)
