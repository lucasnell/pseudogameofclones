...
comm <- function(x, sim, new_starts = NULL, max_t = 1){
	...
	new_starts$N <- x
	restart.sim <- restart_experiment(sim, new_starts = new_starts, max_t = max_t)$all_info[[1]]
...
clone_traj <- function(sim, delta, category = "resistant", max_t = 400, perturb = NULL){

	new.starts <- sim$all_info[[1]]
	pick <- !is.na(new.starts$line) & (new.starts$line == category)
	new.starts[pick, "N"] <- delta * new.starts[pick, "N"]
	
	new.sim <- restart_experiment(sim, new_starts = new.starts, max_t = max_t, perturb = perturb)
...
clone_converge <- function(sim, delta, category = "resistant", max_t = 1e4, tol = 1e-10, perturb = NULL){
	...
	S.resistant <- sum(new.sim$all_info[[1]]$N[!is.na(new.sim$all_info[[1]]$line) & new.sim$all_info[[1]]$line == "resistant"])
	S.susceptible <- sum(new.sim$all_info[[1]]$N[!is.na(new.sim$all_info[[1]]$line) & new.sim$all_info[[1]]$line == "susceptible"])
	test <- (S.resistant/(S.resistant + S.susceptible) > tol)
	return(test)
}

clone_wasp_converge <- function(sim, delta, max_t = 1e4, tol = 1e-8, perturb = NULL, harvesting.length, day.interval){
...
clone_plot <- function(sim, delta.list, labels = "", max_t = 1e3, max_t.converge = 1e4, perturb = NULL){
	d <- sim$aphids
#	d <- d[d$time <= max_t,]
	d$line.type <- paste0(d$line,".", d$type)
	d$line.type[d$line.type == "NA.mummy"] <- "mummy"
	...
	d <- spread(d, "line.type", "N")
	d <- cbind(delta = 0, d, wasps = sim$wasps[,4], converge = T)

	d <- d[d$time <= max_t,]
	
	for(i.delta in delta.list){
		dd <- cbind(clone_traj(sim = sim, delta = i.delta, category = "resistant", max_t = max_t, perturb = perturb), converge = clone_converge(sim = sim, delta = i.delta, category = "resistant", max_t = max_t.converge, perturb = perturb))
		d <- rbind(d, dd)
	}
	...
	plot(prop ~ time, data=d[d$delta == 1 & d$field == 1,], typ="l", ylim=c(0,1), ylab = "Resistant (field 1)", xlab = "Time")
	for(i.delta in delta.list) lines(prop ~ time, data=d[d$delta == i.delta & d$field == 1,], col=col, lty=2-as.numeric(converge))
	text(x=max_t/2, y=.9, labels=labels, cex=2)
	lines(prop ~ time, data=d[d$delta == 1 & d$field == 1,])

	plot(prop ~ time, data=d[d$delta == 1 & d$field == 2,], typ="l", ylim=c(0,1), ylab = "Resistant (field 2)", xlab = "Time")
	for(i.delta in delta.list) lines(prop ~ time, data=d[d$delta == i.delta & d$field == 2,], col=col, lty=2-as.numeric(converge))
	lines(prop ~ time, data=d[d$delta == 1 & d$field == 2,])
	
	plot(wasps ~ time, data=d[d$delta == 1 & d$field == 1,], typ="l", ylim = c(0, max(d$wasps)), ylab = "Wasps (field 1)", xlab = "Time")
	for(i.delta in delta.list) lines(wasps ~ time, data=d[d$delta == i.delta & d$field == 1,], col=col, lty=2-as.numeric(converge))
...
par(mfrow=c(2,1))
plot(N ~ time, data=sims$aphids[sims$aphids$field == 0 & sims$aphids$line == "resistant" & sims$aphids$type == "apterous",], typ="l", col="blue", log="y", ylim=c(1,1500))
lines(N ~ time, data=sims$aphids[sims$aphids$field == 0 & sims$aphids$line == "susceptible" & sims$aphids$type == "apterous",])
lines(wasps ~ time, data=sims$wasps[sims$wasps$field == 0,], col="red")

plot(N ~ time, data=sims$aphids[sims$aphids$field == 1 & sims$aphids$line == "susceptible" & sims$aphids$type == "apterous",], typ="l", log="y", ylim=c(1,1500))
lines(N ~ time, data=sims$aphids[sims$aphids$field == 1 & sims$aphids$line == "resistant" & sims$aphids$type == "apterous",], col="blue")
...
plot(N ~ time, data=sims$aphids[sims$aphids$field == 0 & sims$aphids$line == "resistant" & sims$aphids$type == "apterous",], typ="l", col="blue", ylim=c(700,800), xlim=c(100,700))

plot(N ~ time, data=sims$aphids[sims$aphids$field == 1 & sims$aphids$line == "susceptible" & sims$aphids$type == "apterous",], typ="l", log="y", ylim=c(1,1500))
lines(N ~ time, data=sims$aphids[sims$aphids$field == 1 & sims$aphids$line == "resistant" & sims$aphids$type == "apterous",], col="blue")
lines(wasps ~ time, data=sims$wasps[sims$wasps$field == 1,], col="red")

plot(N ~ time, data=sims$aphids[sims$aphids$field == 1 & sims$aphids$line == "susceptible" & sims$aphids$type == "apterous",], typ="l", ylim=c(1200,1400), xlim=c(100,700))
...
par(mfrow=c(2,1))
plot(N ~ time, data=sim$aphids[sim$aphids$field == 0 & sim$aphids$line == "resistant" & sim$aphids$type == "apterous",], typ="l", col="blue", log="y", ylim=c(.1,1500))
lines(N ~ time, data=sim$aphids[sim$aphids$field == 0 & sim$aphids$line == "susceptible" & sim$aphids$type == "apterous",])
lines(wasps ~ time, data=sim$wasps[sim$wasps$field == 0,], col="red")

plot(N ~ time, data=sim$aphids[sim$aphids$field == 1 & sim$aphids$line == "susceptible" & sim$aphids$type == "apterous",], typ="l", log="y", ylim=c(.1,1500))
lines(N ~ time, data=sim$aphids[sim$aphids$field == 1 & sim$aphids$line == "resistant" & sim$aphids$type == "apterous",], col="blue")
...
C <- jacobian(comm, x, sim=sim)
eig <- eigen(C)
sort(abs(eig$values), decreasing=T)
...
df$aphids.sus.0[df$var==disp.list[15]] <- mean(aphids$N[aphids$field == 0 & aphids$line == "susceptible"])
df$aphids.res.0[df$var==disp.list[15]] <- mean(aphids$N[aphids$field == 0 & aphids$line == "resistant"])
df$aphids.sus.1[df$var==disp.list[15]] <- mean(aphids$N[aphids$field == 1 & aphids$line == "susceptible"])
df$aphids.res.1[df$var==disp.list[15]] <- mean(aphids$N[aphids$field == 1 & aphids$line == "resistant"])
...
for(count in 16:30){	
	...
	df$aphids.sus.0[df$var==disp.list[count]] <- mean(aphids$N[aphids$field == 0 & aphids$line == "susceptible"])
	df$aphids.res.0[df$var==disp.list[count]] <- mean(aphids$N[aphids$field == 0 & aphids$line == "resistant"])
	df$aphids.sus.1[df$var==disp.list[count]] <- mean(aphids$N[aphids$field == 1 & aphids$line == "susceptible"])
	df$aphids.res.1[df$var==disp.list[count]] <- mean(aphids$N[aphids$field == 1 & aphids$line == "resistant"])
...
pdf("Nell fig S9.pdf", width = 10, height = 4)
	...
	lines3D(dd$resistant, dd$susceptible, dd$wasps, colvar = dd$wasps, colkey = F, theta = 0, phi = 0, xlab = "Resistant", ylab = "Susceptible", zlab = "Wasps")
	#points3D(dd$resistant[nrow(dd)], dd$susceptible[nrow(dd)], dd$wasps[nrow(dd)], colkey = F, add = T)
	for(i.delta in delta.list[2:length(delta.list)]) {
		dd <- d[d$delta == i.delta,]
		lines3D(dd$resistant, dd$susceptible, dd$wasps, colvar = dd$wasps, colkey = F, add = T)
		#points3D(dd$resistant[nrow(dd)], dd$susceptible[nrow(dd)], dd$wasps[nrow(dd)], colkey = F, add = T)
	}
	
	dd <- d[d$delta == delta.list[1],]
	lines2D(dd$resistant, dd$susceptible, colvar = dd$wasps, colkey = F, xlab = "Resistant", ylab = "Susceptible")
	points2D(dd$resistant[nrow(dd)], dd$susceptible[nrow(dd)], add=T)
	for(i.delta in delta.list[2:length(delta.list)]) {
		dd <- d[d$delta == i.delta,]
		lines2D(dd$resistant, dd$susceptible, add = T, colvar = dd$wasps, colkey = F)
		points2D(dd$resistant[nrow(dd)], dd$susceptible[nrow(dd)], add=T)
	}
	
	dd <- d[d$delta == delta.list[1],]
	lines2D(dd$time, dd$prop, colvar = dd$wasps, colkey = F, xlab = "Time", ylab = "Proportion resistant")
	for(i.delta in delta.list[2:length(delta.list)]) {
		dd <- d[d$delta == i.delta,]
		lines2D(dd$time, dd$prop, add = T, colvar = dd$wasps, colkey = F)
	}
dev.off()
...
df <- rbind(df, df.save)
write.table(df, "df_Fig4ab_26Oct22.csv", sep=",", row.names=F)
...
w$trough.resistant <- log10(w$trough.resistant + .0001) #/max(w$peak.resistant, na.rm = T)
w$peak.resistant <- log10(w$peak.resistant + .0001) #/max(w$peak.resistant, na.rm = T)
w$trough.susceptible <- log10(w$trough.susceptible + .0001) #/max(w$peak.susceptible, na.rm = T)
w$peak.susceptible <- log10(w$peak.susceptible + .0001) #/max(w$peak.susceptible, na.rm = T)
w$trough.wasps <- log10(w$trough.wasps + .0001) #/max(w$peak.wasps, na.rm = T)
w$peak.wasps <- log10(w$peak.wasps + .0001) #/max(w$peak.wasps, na.rm = T)
...
write.table(df, "df_Fig4cd_26Oct22.csv", sep=",", row.names=F)
...
for(i in 1:3){
	# w[,i + 1 + 3*(0:3)] <- log10(w[,i + 1 + 3*(0:3)]/max(w[,i + 1 + 3*(0:3)], na.rm = T)+.0001)
